...
public class DatabaseManager {
    ...
    static public void registerUser(Connection connection , String name, String surname,
        ...
        try {
            PreparedStatement statement = connection
                    .prepareStatement("INSERT INTO users(name, surname, phonenumber, email, password , level)" +
                    "values(?, ? ,? ,? ,?, ?);");
    ...
    static public void setProjectInfo(Connection connection, String nameP, String phoneP, String emailP){
        try {
            ...
            PreparedStatement statement1 = connection
                    .prepareStatement("INSERT INTO projectinfo(name, phonenumber, email, budget, recording_date) "+
                            "values(?,?,?,?,?)");
    ...
    static public ResultSet getBudget(Connection connection){
        ...
        try {
            PreparedStatement statement = connection
                  .prepareStatement("SELECT budget, recording_date FROM projectinfo ORDER BY recording_date DESC");
            resultSet = statement.executeQuery();
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    static public void saveOrderingProduction(Connection connection, Map<Integer, Integer> map,
        ...
        try {
            PreparedStatement statement1 = connection
                    .prepareStatement("INSERT INTO order_production(id_user,date,price, address) VALUES(?,?,?,?);");
            ...
            for (Map.Entry<Integer,Integer> mSet:map.entrySet()) {
                statement1 = connection
                      .prepareStatement("INSERT INTO resource_for_order_production(id_product, count, id_order) "+
                              "VALUES(?,?,?);");
    ...
    public  static void setOrderingDelivered(Connection connection , int id_order){
        try {
            PreparedStatement statement = connection
                    .prepareStatement("UPDATE order_production SET isdelevered=true WHERE order_production.id=?;");
            statement.setInt(1, id_order);
            statement.execute();
        } catch (SQLException e) {
            e.printStackTrace();
        ...
        try {
            ...
            for (Map.Entry<Integer, Double> mSet : map.entrySet()) {
                statement = connection
                      .prepareStatement("INSERT INTO resource_for_order_resources(id_order, count, id_resource)" +
                                " VALUES (?,?,?);");
                statement.setInt(1, order_id);
